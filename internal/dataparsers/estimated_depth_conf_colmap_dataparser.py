...
@dataclass
class EstimatedDepthConfColmap(Colmap):
    depth_dir: str = "estimated_depths"

    conf_dir: str = "estimated_confs"
    ...
    def instantiate(self, path: str, output_path: str, global_rank: int) -> "EstimatedDepthConfColmapDataParser":
        return EstimatedDepthConfColmapDataParser(path=path, output_path=output_path, global_rank=global_rank, params=self)


class EstimatedDepthConfColmapDataParser(ColmapDataParser):
    def get_outputs(self) -> DataParserOutputs:
        ...
        for image_set in [dataparser_outputs.train_set, dataparser_outputs.val_set]:
            for idx, image_name in enumerate(image_set.image_names):
                ...
                conf_file_path = os.path.join(self.path, self.params.conf_dir, f"{image_name}.npy")
                if os.path.exists(depth_file_path) is False:
                    print("[WARNING] {} does not have a depth file".format(image_name))
                    continue
                    
                if os.path.exists(conf_file_path) is False:
                    print("[WARNING] {} does not have a confidence file".format(image_name))
                    continue
                ...
                image_set.extra_data[idx] = (depth_file_path, depth_scale, conf_file_path)
                loaded_depth_count += 1
            image_set.extra_data_processor = self.load_depth_and_confidence
    ...
    @staticmethod
    def load_depth_and_confidence(depth_conf_info):
        if depth_conf_info is None:
            return None

        depth_file_path, depth_scale, conf_file_path = depth_conf_info
        depth = np.load(depth_file_path) * depth_scale["scale"] + depth_scale["offset"]
        depth = torch.tensor(depth, dtype=torch.float)
        conf = torch.tensor(np.load(conf_file_path), dtype=torch.float)

        return (depth, conf) if depth is not None else (None, None)
